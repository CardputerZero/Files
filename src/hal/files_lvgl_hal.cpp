...
bool initLvglHal(int32_t width, int32_t height)
{
    ...
    if (!disp) {
        spdlog::error("Files HAL: failed to create SDL display");
        return false;
    }

    const float zoom = envFloatOrDefault("FILES_SDL_ZOOM", 1.0f);
    lv_sdl_window_set_resizeable(disp, false);
    lv_sdl_window_set_zoom(disp, zoom);
    lv_sdl_window_set_title(disp, envOrDefault("LV_SDL_WINDOW_TITLE", "Files"));
    spdlog::info("Files HAL: SDL logical display {}x{}, zoom {}", width, height, zoom);
    lv_sdl_mouse_create();
    if (!lv_sdl_keyboard_create()) {
        spdlog::warn("Files HAL: failed to create SDL keyboard input");
    }
    return true;
#elif LV_USE_LINUX_FBDEV
    (void)width;
    ...
    if (!disp) {
        spdlog::error("Files HAL: failed to create framebuffer display");
        return false;
    }

    const char* device = envOrDefault("LV_LINUX_FBDEV_DEVICE", "/dev/fb0");
    if (lv_linux_fbdev_set_file(disp, device) != LV_RESULT_OK) {
        spdlog::error("Files HAL: failed to open framebuffer {}", device);
        return false;
    }
    return true;
#else
    spdlog::error("Files HAL: no LVGL display driver enabled");
    return false;
#endif
}
